...
# - uploads and runs the on-host-run-tempest.sh scripts
#
...
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$thisdir/on-host-run-tempest.sh" "root@$server:$tmpdir"
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$thisdir/on-domu-run-tempest.sh" "root@$server:$tmpdir"

#
# Run tests
#
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "root@$server" "$tmpdir/on-host-run-tempest.sh"
